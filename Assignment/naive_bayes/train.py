	...
	for d, dic in enumerate(dicts):
		for k, v in dic.items():
			if k not in condprob:
				condprob[k] = {0: 0, 1: 0}
				# if alpha is not 0:
				condprob[k][1-d] = math.log(Decimal(alpha)/(nOfTerms[d]+alpha*len(dicts[d])))
			condprob[k][d] = math.log(Decimal(v+alpha)/(nOfTerms[d]+alpha*len(dicts[d])))

# Starts the training phase.
def doTrain(alpha):
	cwd = os.getcwd()
	reader("train/pos/", 1) #positives
	os.chdir(cwd)
	reader("train/neg/", 0) #negatives
	os.chdir(cwd)
	calcLikeli(alpha)
